...
const MovieGenre = () => {
  const { id, movieGenreName } = useParams();
    ...
    const getMovieGenreList = async () => {
      try {
        const response = await fetch(
          `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=${id}&api_key=6a77f5a2cdf1a150f808f73c35533a92`
        );
  ...
  const loadMore = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=${i}&sort_by=popularity.desc&with_genres=${id}&api_key=6a77f5a2cdf1a150f808f73c35533a92`
      );
      const data = await response.json();
  ...
  return (
    <div>
      {/* <Background /> */}
      <h2 className="genreTitle">Genre: {movieGenreName}</h2>
